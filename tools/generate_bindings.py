...
def autogenerate() -> None:
    ...
    include_dir = repository_dir + "/build/vcpkg_installed/x64-windows/include/Inventor"
    header_files = [include_dir + "/nodes/SoNode.h"]
    header_files = glob.glob(include_dir + "/*.h", recursive=True)

    logger.debug(f"header fiels: {'\n'.join(header_files)}")

    output_cpp_pydef_file = repository_dir + "/_pydef_pybind11/pybind_Inventor.cpp"
    with open(output_cpp_pydef_file, "a") as f:
        f.write(
            """
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  AUTOGENERATED CODE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
"""
    ...
    output_stub_pyi_file = repository_dir + "/_stubs/PyInventor/__init__.pyi"
