...
class TDragger : public SoDragger
{
  ...
  public:
    ...
    TDragger();
    SoSFVec3f translation; //!< set from outside and used from outside for
                           //!< single precision.
    SoSFDouble
        translationIncrement; //!< set from outside and used for rounding.
    SoSFInt32
        translationIncrementCount; //!< number of steps. used from outside.
...
class TPlanarDragger : public SoDragger
{
  ...
  public:
    ...
    SoSFDouble
        translationIncrement; //!< set from outside and used for rounding.
    SoSFInt32
        translationIncrementXCount; //!< number of steps. used from outside.
    SoSFInt32
        translationIncrementYCount; //!< number of steps. used from outside.
    SoSFFloat autoScaleResult;      //!< set from parent dragger.
...
class RDragger : public SoDragger
{
  ...
  public:
    static void initClass();
    RDragger();
    SoSFRotation rotation; //!< set from outside and used from outside for
                           //!< single precision.
 ...
 */
class SoFCCSysDragger : public SoDragger
{
  ...
  public:
    ...
    SoSFVec3f translation; //!< initial translation and reflects single
                           //!< precision movement.
    SoSFDouble translationIncrement; //!< set from outside used for rounding.
    SoSFInt32 translationIncrementCountX; //!< used from outside for translation
                                          //!< x steps.
    SoSFInt32 translationIncrementCountY; //!< used from outside for translation
                                          //!< y steps.
    SoSFInt32 translationIncrementCountZ; //!< used from outside for translation
                                          //!< z steps.

    SoSFRotation
        rotation; //!< initial rotation and reflects single precision movement.
    SoSFDouble
        rotationIncrement; //!< radians set from outside and used for rounding.
    SoSFInt32
        rotationIncrementCountX; //!< used from outside for rotation x steps.
    SoSFInt32
        rotationIncrementCountY; //!< used from outside for rotation y steps.
    SoSFInt32
        rotationIncrementCountZ; //!< used from outside for rotation z steps.

    void clearIncrementCounts(); //!< used to reset after drag update.

    /*! @brief Overall scale of dragger node.
     *
     * When using autoscale mode, this represents normalized device coordinates
     * (0.0 to 1.0). A value of 0.05 is a good place to start. When NOT using
     * autoscale mode, scale represents a traditional scale and a value of 1.0
     * is a good place to start.
     */
    SoSFFloat draggerSize;
    SoSFFloat autoScaleResult; //!< result of autoscale calculation and used by
                               //!< childdraggers. Don't use.

    SoIdleSensor idleSensor; //!< might be overkill, but want to make sure of
                             //!< performance.
    void setUpAutoScale(
        SoCamera *cameraIn); //!< used to setup the auto scaling of dragger.

    void setAxisColors(unsigned long x, unsigned long y,
                       unsigned long z); //!< set the axis colors.
    ...
    bool
    isShownPlanarTranslationXY(); //!< is xy planar translation dragger shown.
    bool
    isShownPlanarTranslationYZ(); //!< is yz planar translation dragger shown.
    bool
    isShownPlanarTranslationZX(); //!< is zx planar translation dragger shown.
    bool
    isHiddenPlanarTranslationXY(); //!< is xy planar translation dragger hidden.
    bool
    isHiddenPlanarTranslationYZ(); //!< is yz planar translation dragger hidden.
    bool
    isHiddenPlanarTranslationZX(); //!< is zx planar translation dragger hidden.
  ...
  protected:
    ...
    static void
    idleCB(void *data,
           SoSensor *); //!< scheduled from cameraCB to auto scale dragger.
